...
from local_catalog_screening import chunked_star_search, streaming_star_search
...
class camera(hardware):
    def __init__(self,par_camera,name = 'simulator_cam'):
        ...
        default_par = {
            ...
            "readout_time_s": 0.3448275862068966,
            "full_well_capacity_ke": 200000,
            "bias_level": 400,
            "CCD_temperature": 265.15
        }
        user_par = par_camera
        merged = {**default_par, **user_par}
        self.initpar = merged  # initpar: dictionary
        self.idle_time = -1
...
class mount(hardware):
    def __init__(self,par_mount,name = 'simulator_mount'):
        ...
        default_par = {
            "tracking_mode": "alt-az",
            "tracking_speed_deg_s_1": 5,
            "stable_time_s": 5,
            "goto_error_arcsec": 2,
            "tracking_error_arcsec_min_1": 0.1
        }
        user_par = par_mount
        merged = {**default_par, **user_par}
        self.initpar = merged
...
class telescope:
    def __init__(self,mount,camera,par_telescope ,name = 'simulator_telescope'):
        ...
        self.camera = camera
        default_par = {
            "telescope_name": "simulator_telescope"
        }
        user_par = par_telescope
        merged = {**default_par, **user_par}
        for i in merged:
            setattr(self,i,merged[i])
            ...
            if self.time > r['jd_utc_end']:
                ...
                with tqdm(total=total_frames, desc=r['target_name']) as pbar:
                    while self.time < r['jd_utc_end'] and (r['n_max_frames']<=0 or ct_name[r['target_name']]< r['n_max_frames']):
                        ...
                        target_name = str(r['observation_order']) + '_' + str(r['target_name'])
                        target_dir = Path(self.output) / target_name
...
CONTAINS(
    ...
    def generate_photon(self):
        ...
        # allow user-defined stray light field
        if self.user_stray_field is not None:
            if isinstance(self.user_stray_field, str):
                if not os.path.exists(self.user_stray_field):
                    raise ValueError(f"user_stray_field not found: {self.user_stray_field}")
                ext = os.path.splitext(self.user_stray_field)[1].lower() # .lower() is uniformly converted to lowercase.
                if ext in (".fits", ".fit", ".fts"):
                    try:
                        arr = fits.getdata(self.user_stray_field)
                    except Exception as e:
                        raise ValueError(f"failed to read FITS stray field '{self.user_stray_field}': {e}")
                elif ext in (".npy", ".npz"): # npy is a single array and.npz is a compressed package. 
                    try:
                        arr = np.load(self.user_stray_field)
                        if isinstance(arr, np.lib.npyio.NpzFile):
                            keys = list(arr.keys()) 
                            if not keys:
                                raise ValueError(f"npz file '{self.user_stray_field}' contains no arrays")
                            arr = arr[keys[0]] # For npz files, by default, the first array saved within them is taken.
                    except Exception as e:
                        raise ValueError(f"failed to read numpy stray field '{self.user_stray_field}': {e}")
                else:
                    raise ValueError(f"unsupported stray field extension: '{ext}'")
            else:
                raise ValueError(f"user_stray_field must be a path, got {type(self.user_stray_field)}")

            arr = np.asarray(arr, dtype=np.float64)
            if arr.ndim != 2:
                raise ValueError("loaded stray field is scalar, expected 2D array")

            # resize to img shape (cv2 dsize = (width, height))
            try:
                stray_field = cv2.resize(arr, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_LINEAR).astype(np.float64)
                print(f"Resized stray field from original shape {arr.shape} to image shape {img.shape}.")
            except Exception as e:
                raise ValueError(f"failed to resize stray field to image shape {img.shape}: {e}")
            if np.any(stray_field < 0):
                print("Warning: stray field contains non-positive values. Clipping to 0.")
                print(f"the total number of non-positive values: {np.sum(stray_field <= 0)}")
                stray_field = np.clip(stray_field, 0, None)
            
            total_photon = (sky + flux_variable_star  + satellite + asteroid + galaxy + stray_field) * scintillation
        else:
